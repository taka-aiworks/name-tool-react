...
3. **キャラクター表示切り替え強化**
   - 顔のみ/上半身/全身の切り替えをより直感的に
   - UI上での切り替えボタンを分かりやすく配置
...
```
src/
├── types.ts                    # 共通型定義（onCharacterSelect追加済み）
...
│       ├── sceneTemplates.ts   # シーンテンプレート定義（バグあり）
│       └── renderers/
│           ├── BubbleRenderer.tsx    # 吹き出し描画・操作
│           ├── CharacterRenderer.tsx # キャラクター描画・操作（改善済み）
...
### 📈 重要度：中（機能追加）
1. **キャラクターサイズ制限解除**
   - CharacterRenderer.tsx のスケール上限撤廃
...
3. **キャラクター詳細設定UI**
   - CharacterDetailPanel.tsx の機能拡張
   - 向き・表情・視線・ポーズの選択UI
...
2. **テスト** → **コミット** → **GitHub保存** を習慣化
3. **ユーザーフィードバック**を積極的に収集
